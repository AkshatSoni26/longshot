...
| Known pipeline error (`FetchError`/`SummarizeError` — e.g. LLM unreachable, fetch timeout) | Inner pipeline emits `ErrorEvent`; `run_job` catches and returns `TaskResult(status=ERROR)` — no broker retry | SSE `error` event then close |
